...
int
mac_ntop(char *addr, char *dst, size_t s)
{
...
void
dhcp_msg_dump(FILE * stream, struct dhcp_msg * msg)
{
	fprintf(stream,
		"DHCP message:\n"
		"\tOP %hhu [%s]\n"
...
void
msg_debug(struct dhcp_msg * msg, int dir)
{
	if (dir == 0)
...
void
get_config(struct fb_config * config, const struct arguments const * args)
{
...
int
do_listen(struct fb_config * config)
{
	int 	fd;
	const int one = 1;
...
static 	uint32_t
netmask_from_prefixlen(uint8_t prefixlen)
{
	return htonl(0xFFFFFFFFU - (1 << (32 - prefixlen)) + 1);
}

uint8_t *
dhcp_opt_add_lease(uint8_t * options, size_t * _send_len, struct dhcp_lease * lease)
{
	size_t 	send_len = 0;
	...
	if (lease->prefixlen > 0) {
		...
		options[1] = 4;
		ARRAY_COPY((options + 2), (uint8_t *) ((uint32_t[]) {
			netmask_from_prefixlen(lease->prefixlen)
		}), 4);
...
discover_cb(int sock, struct dhcp_msg * msg)
{
	...
	*options = DHCP_OPT_END;
	...
	if (debug)
		msg_debug(&((struct dhcp_msg) {
		.data = send_buffer,.length = send_len
		}), 1);
	int 	ret = sendto(sock,
	...
	if (ret < 0)
		err(errno, "Could not send DHCPOFFER");
...
req_cb(int sock)
{
	...
	char 	ciaddr[INET_ADDRSTRLEN], yiaddr[INET_ADDRSTRLEN], siaddr[INET_ADDRSTRLEN],
		giaddr [INET_ADDRSTRLEN], chaddr[MAC_ADDRSTRLEN], srcaddr[INET_ADDRSTRLEN];
